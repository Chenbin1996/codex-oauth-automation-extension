...
- 展示 `Plus 模式` 开关与 Plus 支付方式配置；支付方式支持 PayPal / GoPay / GPC，PayPal 展示账号池下拉与添加按钮，GoPay 展示手机号和 PIN，GPC 展示 API 地址、API Key、专用手机号、OTP 渠道、本地 OTP helper 开关与 URL、PIN；步骤列表切换为 Plus 模式步骤定义，普通模式的注册成功等待步骤不再显示或执行，登录验证码步骤会移动到 Plus 可见第 11 步
